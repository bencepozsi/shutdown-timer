...
impl Application {
    pub fn new() -> (Self, Task<Message>) {
        (Self {
            ...
            finish_time: SystemTime::now().sub(Duration::from_hours(1)).into()
        }, Task::none())
    }

    pub fn view(&self) -> Element<'_, Message> {
        let current_datetime: DateTime<Local> = SystemTime::now().into();
        let time_str = format!("Current time: {}", current_datetime.format("%H:%M:%S").to_string());
        let shutdown_str = format!("Shutdown at: {}", if self.countdown {
        ...
        let action_button = (if self.countdown {stop_button} else {start_button})
            .width(100);
        ...
        ].align_x(Alignment::Center).spacing(12).width(WINDOW_WIDTH).into()
    }
    ...
    pub fn update(&mut self, message: Message) -> Task<Message>{
        match message {
            ...
            Message::StartTimer => {
                let elapsed_secs = make_secs(self.hours.clone(), self.minutes.clone(), self.seconds.clone());
                if elapsed_secs > 0 {
                    self.start_time = SystemTime::now().into();
                    self.finish_time = self.start_time.clone().add(Duration::from_secs(elapsed_secs));
                    self.countdown = true;
                }
            },
            Message::StopTimer => {
                self.finish_time = self.start_time.clone().sub(Duration::from_hours(1));
